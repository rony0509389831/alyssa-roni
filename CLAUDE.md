...
- **`shade_level` + `shade_factor` exponent (2 רמות, לא 3, מ-2026-07-18):** AGENT מחלץ `shade_level` (null/"short"/"max") → ממופה ל-`shade_factor` מספרי (1.0/3.0 בהתאמה) → `TCI^shade_factor` מגביר את ניגודיות המשקלות (shade_factor=3.0: TCI 2→8, TCI 8→512). **"מאוזן" (2.0) הוסרה** — בבדיקה אמפירית על מסלולים אמיתיים היא כמעט תמיד התכנסה לאותו מסלול בדיוק כמו "הרבה צל" (או לפעמים כמו "מעט צל"), בלי הבדל תפעולי אמיתי; מגבלה מבנית של המרחק בין עוצמות-צל סמוכות ברוב הטיולים, לא באג. ברירת המחדל (גם ב-UI וגם ב-agent כשלא צוינה העדפה) היא "מעט צל"; כל בקשת-צל מפורשת או כללית ("מוצל"/"ירוק"/"הכי מוצל") ממופה ל-"הרבה צל". כלל-האזהרה האוטומטי על "חם באמצע" (warm→balanced) הוסר גם הוא — נשאר רק כלל ה"חם מאוד" (hot→max).
