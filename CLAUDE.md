...
- **שדות פלט (7):** `origin`, `destination`, `hour` (0.0–23.99 decimal; null=לא צוינה שעה כלל), `date` (YYYY-MM-DD; null=היום), `mode` ("shaded"/"fast"), `shade_level` (null/"short"/"max" — **רק 2 רמות מ-2026-07-18**, "balanced" הוסרה: לא הצדיקה בפועל רמה נפרדת, ר' "תקרת עיקוף" למטה), `recommendation` (הסבר אוטומטי ≤100 תווים)
...
- **תקרת עיקוף פר-רמה (`_TIER_DETOUR_CAPS`):** המסלול המוצל לא יעלה על תקרת-יחס-אורך/תקרת-דקות של הרמה שנבחרה (המחמיר מביניהן) — "מעט צל" (1.0): **1.20×/10-דק'** (הורחב מ-1.10×/5-דק', 2026-07-18 מאוחר יותר); "הרבה צל" (3.0): 1.60×/30-דק' (ללא שינוי). `plan_route` מוריד את `shade_factor` בהדרגה (צעדי 0.1, רצפה משותפת 0.1 לשתי הרמות) עד שהמסלול עומד בתקרה.
  **הרחבת "מעט צל" (2026-07-18 מאוחר יותר):** אומת חי (לא ניחוש) שהרחבת האחוז לבד (10%→20%, בלי לגעת בדקות) היא **no-op** על טיולים מעל כ-50 דק' — איבר-הדקות הקבוע (`+5`) כבר המחמיר שם (`1+5/50.4≈1.10`), אז תקרת-האחוזים החדשה אף פעם לא מגיעה לתפוס. חייבים להגדיל את **שני** האיברים ביחד. על מסלול אמיתי (~50 דק') זה פתח `shade_factor=0.3` במקום `0.2` (עיקוף ~17% במקום ~10%) וקירר את ה-TCI ב-**1.4 נקודות** — שיפור אמיתי. על מסלול אחר (קפיצה חדה בין קצר-חשוף לארוך-מוצל, בלי "אמצע") זה לא שינה כלום — תלוי-גאומטריית-מסלול, לא קבוע אוניברסלי.
  **מדרג-נפילה בשלושה שלבים (2026-07-18, ערב)** — כי תקרת-הדקות הקבועה מתכווצת *יחסית* ככל שהטיול ארוך יותר (מעל ~50 דק' היא הופכת למגבילה מהיחס%: לדוגמה טיול 72 דק' מקבל בפועל רק ~42% עיקוף במקום 60% המובטחים ל"הרבה צל"), כך שרמה יכולה להיכשל בתקציב הצר שלה גם כשצל אמיתי זמין ברמה השנייה. `plan_route` כבר לא מוותר על צל בפעם הראשונה שהרמה הנבחרת נכשלת:
  1. מנסים את הרמה שנבחרה (כרגיל).
  2. נכשלה? מנסים את "הרבה צל" (התקרה הרחבה ביותר) לפני שמוותרים — הצלחה כאן = `fallback="tier_escalated"` (מגולה למשתמש: "הרחבנו את תקציב העיקוף...").
  3. גם זו נכשלה? בודקים אם הניסיון הכי טוב שנמצא (מכל אחת מ-2 הרמות) בכל זאת קריר מהמסלול המהיר בפועל. אם כן — מוצג כ`fallback="best_effort_over_budget"` (חורג מהתקציב הרגיל, אך אמיתי ומועיל, לא נזרק). רק אם אף ניסיון לא קריר מהמהיר → נפילה כנה למהיר (`fallback="detour_cap_unreachable"`) — הפרשנות ה"אין שום מסלול-צל שמשתלם" האמיתית, לא רק "התקרה הצרה נכשלה".
  לעולם לא מסלול "מוצל" שקרי (חם מהמהיר) ולעולם לא זריקת חיפוש-צל שהצליח חלקית בלי להראות כלום — שני אלה היו הבאגים שהובילו לשלב הזה.
...
- **`shade_level` + `shade_factor` exponent (2 רמות, לא 3, מ-2026-07-18):** AGENT מחלץ `shade_level` (null/"short"/"max") → ממופה ל-`shade_factor` מספרי (1.0/**2.0** בהתאמה) → `TCI^shade_factor` מגביר את ניגודיות המשקלות (shade_factor=2.0: TCI 2→4, TCI 8→64). **"הרבה צל" הורד מ-exponent 3.0 ל-2.0 (2026-07-18, ב-fork של rony):** אומת חי שב-3.0 ה-A* מייצר מסלולים משוננים עם backtrack (עד ~81מ' אחורה מהיעד, "העיקול המוזר ביהודה הלוי") עבור שיפור-TCI זניח (~0.3 נק'); ב-exponent ≤2.5 המסלול חלק (0 backtrack) עם כמעט אותו צל. 2.0 = הערך הבטוח מתחת ל"מדרון" 2.5→3.0. גם `_TIER_DETOUR_CAPS` ו-`WIDEST_TIER` עודכנו ל-2.0. **"מאוזן" (2.0) הוסרה** — בבדיקה אמפירית על מסלולים אמיתיים היא כמעט תמיד התכנסה לאותו מסלול בדיוק כמו "הרבה צל" (או לפעמים כמו "מעט צל"), בלי הבדל תפעולי אמיתי; מגבלה מבנית של המרחק בין עוצמות-צל סמוכות ברוב הטיולים, לא באג. ברירת המחדל (גם ב-UI וגם ב-agent כשלא צוינה העדפה) היא "מעט צל"; כל בקשת-צל **לא-ממותנת** מפורשת או כללית ("מוצל"/"ירוק"/"הכי מוצל") ממופה ל-"הרבה צל". כלל-האזהרה האוטומטי על "חם באמצע" (warm→balanced) הוסר גם הוא — נשאר רק כלל ה"חם מאוד" (hot→max). **בקשת-צל ממותנת → "מעט צל" (2026-07-18, ב-fork של rony):** "קצת צל"/"קצת מוצל"/"a little shade" ממופה ל-`short` ולא ל-`max` — במערכת 2-הרמות "קצת" קרוב ל"מעט" יותר מ"מקסימום". מיושם דו-שכבתי כמו כלל-הלילה: הוראת פרומפט + override דטרמיניסטי ב-`_validate` (`_SHADE_DIMINISHERS`, גובר על max רק כשזוהתה בקשת-צל). **סוטה מהחלטת-הבסיס "כל צל → max" — קיים כרגע רק ב-fork.**
