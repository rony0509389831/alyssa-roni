...
st.markdown(
    ...
    <style>
        ...
        .shady-fact-card {
            position: relative; z-index: 2; max-width: 640px; margin: 6px 0 18px;
            padding: 10px 18px; border-radius: 999px; background: var(--shade-soft);
    ...
    if not _AGENT:
        ...
        if _params.get("error"):
            ...
            _shade_label_display = {
                "short": "הכי מהר", "balanced": "מאוזן", "max": "צל",
            }.get(_sl or "", "")
...
_shade_label = st.radio(
    ...
    help="הכי מהר = מוטה לצל הכי פחות; מאוזן = ברירת מחדל; צל = צל מקסימלי גם במחיר עיקוף ניכר",
)
...
with st.expander("📎 חומרי רקע ותיעוד (למטרות הערכה בקורס)", expanded=False):
    ...
    with st.expander("ℹ️ אודות SHADY"):
        ...
        | אפשרות | משמעות |
        |--------|---------|
        | 🚶 הכי מהר | מינימום התחשבות בצל |
        | 🌤 מאוזן | איזון בין אורך לנוחות |
        | 🌳 צל | הצל מנצח הכל, גם במחיר מסלול ארוך יותר |
