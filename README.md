...
| # | בדיקה | תוצאה |
|---|-------|--------|
| 1 | ניצחון על baseline | ✅ שיפור ×15.9 (RMSE 1.790 → 0.113) |
...
2. הזינו נקודת מוצא ויעד בתל אביב
3. המודל מחזה TCI לכל 58,360 קשתות הגרף בקריאת `predict` אחת
4. Dijkstra מוצא את המסלול בעל עלות TCI כוללת מינימלית (`weight = TCI × length`)
5. המסלול מוצג בירוק + מדד **"חשיפה ממוצעת לשמש"** (TCI ממוצע, 1–10)

> השוואה: מצב "⚡ הכי מהיר (OSRM)" מחזיר את מסלול OSRM הקלאסי ללא שיקולי צל — ניתן להשוות בין שניהם.
...
- אפשרות לבחור **שעת יציאה עתידית** — עד 48 שעות קדימה — תוך שליפת תחזית מזג אוויר (Open-Meteo forecast API) וחישוב מיקום השמש לאותו רגע עם PySolar.
- ברירת מחדל תישאר שעה נוכחית.
